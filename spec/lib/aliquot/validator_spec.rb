...
shared_examples 'Validator Spec' do
  context 'TokenSchema' do
    ...
    context 'signature' do
      it 'must exist' do
        ...
        is_expected.to eq({signature: ['is missing']})
      end

      it 'must be filled' do
        generator.signature = ''
        is_expected.to eq({signature: ['must be filled']})
      end

      it 'must be a string' do
        generator.signature = 123
        is_expected.to eq({signature: ['must be a string']})
      end

      it 'must be base64' do
        generator.signature = 'not base64'
        is_expected.to eq({signature: ['must be Base64']})
      end

      it 'must be asn1' do
        generator.signature = Base64.strict_encode64('base64')
        is_expected.to eq({signature: ['must be base64 encoded asn1 value']})
      end
    end
    ...
    context 'protocolVersion' do
      it 'must exist' do
        ...
        is_expected.to eq({protocolVersion: ['is missing']})
      end

      it 'must be filled' do
        token['protocolVersion'] = ''
        is_expected.to eq({protocolVersion: ['must be filled']})
      end

      it 'must be a string' do
        token['protocolVersion'] = 123
        is_expected.to eq({protocolVersion: ['must be a string']})
      end
    end
    ...
    context 'signedMessage' do
      it 'must exist' do
        ...
        is_expected.to eq({signedMessage: ['is missing']})
      end

      it 'must be filled' do
        token['signedMessage'] = ''
        is_expected.to eq({signedMessage: ['must be filled']})
      end

      it 'must be a string' do
        generator.signed_message = 123
        is_expected.to eq({signedMessage: ['must be a string']})
      end

      it 'must be valid JSON' do
        generator.signed_message = 'not valid json'
        is_expected.to eq({signedMessage: ['must be valid JSON']})
      end
    end
  end
  ...
  context 'SignedMessageScheme' do
    ...
    context 'encryptedMessage' do
      it 'must exist' do
        ...
        is_expected.to eq({encryptedMessage: ['is missing']})
      end

      it 'must be filled' do
        input['encryptedMessage'] = ''
        is_expected.to eq({encryptedMessage: ['must be filled']})
      end

      it 'must be a string' do
        generator.encrypted_message = 123
        is_expected.to eq({encryptedMessage: ['must be a string']})
      end

      it 'must be base64' do
        generator.encrypted_message = 'not base64'
        is_expected.to eq({encryptedMessage: ['must be Base64']})
      end
    end
    ...
    context 'ephemeralPublicKey' do
      it 'must exist' do
        ...
        is_expected.to eq({ephemeralPublicKey: ['is missing']})
      end

      it 'must be filled' do
        input['ephemeralPublicKey'] = ''
        is_expected.to eq({ephemeralPublicKey: ['must be filled']})
      end

      it 'must be a string' do
        generator.ephemeral_public_key = 123
        is_expected.to eq({ephemeralPublicKey: ['must be a string']})
      end

      it 'must be base64' do
        generator.ephemeral_public_key = 'not base64'
        is_expected.to eq({ephemeralPublicKey: ['must be Base64']})
      end
    end
  end
  ...
  context 'PaymentMethodDetailsSchema' do
    ...
    context 'pan' do
      it 'must exist' do
        input.delete('pan')
        is_expected.to eq({pan: ['is missing']})
      end

      it 'must be filled' do
        generator.pan = ''
        is_expected.to eq({pan: ['must be filled']})
      end

      it 'must be integer string' do
        generator.pan = 'no integers here'
        is_expected.to eq({pan: ['must be string encoded integer']})
      end

      it 'must be a pan' do
        generator.pan = '1121412908091872401284'
        is_expected.to eq({pan: ['must be a pan']})
      end
    end
    ...
    context 'expirationMonth' do
      it 'must exist' do
        ...
        is_expected.to eq({expirationMonth: ['is missing']})
      end

      it 'must be filled' do
        generator.expiration_month = ''
        is_expected.to eq({expirationMonth: ['must be filled']})
      end

      it 'must be an integer' do
        generator.expiration_month = 'a string'
        is_expected.to eq({expirationMonth: ['must be an integer']})
      end

      it 'must be a month' do
        generator.expiration_month = 13
        is_expected.to eq({expirationMonth: ['must be a month (1..12)']})
      end
    end
    ...
    context 'expirationYear' do
      it 'must exist' do
        ...
        is_expected.to eq({expirationYear: ['is missing']})
      end

      it 'must be filled' do
        generator.expiration_year = ''
        is_expected.to eq({expirationYear: ['must be filled']})
      end

      it 'must be an integer' do
        generator.expiration_year = 'a string'
        is_expected.to eq({expirationYear: ['must be an integer']})
      end

      it 'must be a year' do
        generator.expiration_year = 19993
        is_expected.to eq({expirationYear: ['must be a year (2000..3000)']})
      end
    end
    ...
    context 'authMethod' do
      it 'must exist' do
        ...
        is_expected.to eq({authMethod: ['is missing']})
      end

      it 'must be filled' do
        generator.auth_method = ''
        is_expected.to eq({authMethod: ['must be filled']})
      end

      it 'must be a string' do
        generator.auth_method = 123
        is_expected.to eq({authMethod: ['must be a string']})
      end

      it 'must be PAN_ONLY or CRYPTOGRAM_3DS' do
        generator.auth_method = 'INVALID_AUTH_METHOd'
        is_expected.to eq({authMethod: ['must be one of: PAN_ONLY, CRYPTOGRAM_3DS']})
      end
    end
    ...
    context 'cryptogram' do
      context 'when PAN_ONLY' do
        it 'must not exist' do
          input['cryptogram'] = '05'
          is_expected.to eq({cryptogram: ['cannot be defined']})
        end
      end
      ...
      context 'when CRYPTOGRAM_3DS' do
        ...
        it 'must exist' do
          ...
          is_expected.to eq({cryptogram: ['is missing']})
        end

        it 'must be filled' do
          generator.cryptogram = ''
          is_expected.to eq({cryptogram: ['must be filled']})
        end

        it 'must be a string' do
          generator.cryptogram = 123
          is_expected.to eq({cryptogram: ['must be a string']})
        end
      end
    end
    ...
    context 'eciIndicator' do
      context 'when PAN_ONLY' do
        it 'must not exist' do
          ...
          is_expected.to eq({eciIndicator: ['cannot be defined']})
        end
      end
      ...
      context 'when CRYPTOGRAM_3DS' do
        ...
        it 'must be a string' do
          ...
          is_expected.to eq({eciIndicator: ['must be a string']})
        end

        it 'must be an ECI' do
          generator.eci_indicator = 'ff'
          is_expected.to eq({eciIndicator: ['must be an ECI']})
        end
      end
    end
  end
  ...
  context 'EncryptedMessageSchema' do
    ...
    context 'messageExpiration' do
      it 'must exist' do
        ...
        is_expected.to eq({messageExpiration: ['is missing']})
      end

      it 'must be filled' do
        generator.message_expiration = ''
        is_expected.to eq({messageExpiration: ['must be filled']})
      end

      it 'must be a string' do
        generator.message_expiration = 123
        is_expected.to eq({messageExpiration: ['must be a string']})
      end

      it 'must be an integer string' do
        generator.message_expiration = 'not integer string'
        is_expected.to eq({messageExpiration: ['must be string encoded integer']})
      end
    end
    ...
    context 'messageId' do
      it 'must exist' do
        ...
        is_expected.to eq({messageId: ['is missing']})
      end

      it 'must be filled' do
        generator.message_id = ''
        is_expected.to eq({messageId: ['must be filled']})
      end

      it 'must be a string' do
        generator.message_id = 123
        is_expected.to eq({messageId: ['must be a string']})
      end
    end
    ...
    context 'paymentMethod' do
      it 'must exist' do
        ...
        is_expected.to eq({paymentMethod: ['is missing']})
      end

      it 'must be filled' do
        generator.payment_method = ''
        is_expected.to eq({paymentMethod: ['must be filled']})
      end

      it 'must be a string' do
        generator.payment_method = 123
        is_expected.to eq({paymentMethod: ['must be a string']})
      end

      it 'must be CARD' do
        generator.payment_method = 'RANDOM'
        is_expected.to eq({paymentMethod: ['must be equal to CARD']})
      end
    end
    ...
    context 'paymentMethodDetails' do
      it 'must exist' do
        ...
        is_expected.to eq({paymentMethodDetails: ['is missing']})
      end

      it 'must be a JSON object' do
        generator.payment_method_details = 'not a json object'
        is_expected.to eq({paymentMethodDetails: ['must be a JSON object']})
      end
    end
  end
end
  ...
  context 'ECv2' do
    ...
    context 'SignedKeySchema' do
      ...
      context 'keyExpiration' do
        it 'must exist' do
          ...
          is_expected.to eq({keyExpiration: ['is missing']})
        end

        it 'must be filled' do
          input['keyExpiration'] = ''
          is_expected.to eq({keyExpiration: ['must be filled']})
        end

        it 'must be integer string' do
          generator.key_expiration = 'not digits'
          is_expected.to eq({keyExpiration: ['must be string encoded integer']})
        end
      end
      ...
      context 'keyValue' do
        it 'must exist' do
          ...
          is_expected.to eq({keyValue: ['is missing']})
        end

        it 'must be filled' do
          input['keyValue'] = ''
          is_expected.to eq({keyValue: ['must be filled']})
        end

        it 'must be ec_public_key' do
          generator.key_value = 'not EC public key'
          is_expected.to eq({keyValue: ['must be an EC public key']})
        end
      end
    end
    ...
    context 'IntermediateSigningKeySchema' do
      ...
      context 'signedKey' do
        it 'must exist' do
          ...
          is_expected.to eq(signedKey: ['is missing'])
        end

        it 'must be filled' do
          token['intermediateSigningKey']['signedKey'] = ''
          is_expected.to eq(signedKey: ['must be filled'])
        end

        it 'must be a string' do
          generator.signed_key_string = 122
          is_expected.to eq(signedKey: ['must be a string'])
        end

        it 'must be valid json' do
          generator.signed_key = 'Invalid JSON'
          is_expected.to eq(signedKey: ['must be valid JSON'])
        end
      ...
      context 'signatures' do
        it 'must exist' do
          ...
          is_expected.to eq(signatures: ['is missing'])
        end

        it 'must be filled' do
          generator.signatures = ''
          is_expected.to eq(signatures: ['must be filled'])
        end

        it 'must be an array' do
          generator.signatures = 'Not an array'
          is_expected.to eq(signatures: ['must be an array'])
        end

        it 'entries must be base64' do
          generator.signatures = ['Not base64']
          is_expected.to eq(signatures: {0 => ['must be Base64']})
        end
    ...
    context 'intermediateSigningKey' do
      ...
      it 'must exist' do
        ...
        is_expected.to eq({intermediateSigningKey: ['is missing']})
      end

      it 'must be a JSON object' do
        generator.intermediate_signing_key = 'Not a JSON object'
        is_expected.to eq({intermediateSigningKey: ['must be a JSON object']})
      end
    end
  end
end
